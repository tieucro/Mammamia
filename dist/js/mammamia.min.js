...
var Mammamia = {
    products: [],
    plugins: {},
    settings: { numColumns: 3, brand: "Mammamia", mode: "JSON", JSONFile: "products.json", fadeSpeed: 200, buttonColor: null, backgroundColor: null, textColor: null, container: $(".Mammamia_container"), cartContainer: $(".Mammamia_cart_container"), rowClass: "Mammamia_row_", columnWidthClasses: { 1: "", 2: "one-half", 3: "one-third" } },
    productPageOptions: ["OneOfAKind"],
    extend: function(a, b, c) { var d; "undefined" == typeof b && (b = a, a = Mammamia); for (d in b) Object.prototype.hasOwnProperty.call(b, d) && (a[d] = b[d]); return c(), a },
    render: function(a, b) {
        var c = a.split("/")[0],
            d = {
                "": function() { Mammamia.renderProducts(Mammamia.products, b) },
                "#product": function() {
                    var c = a.split("#product/")[1].trim();
                    Mammamia.renderSingleProduct(c, b)
                },
                "#cart": function() { Mammamia.renderCart(b) }
            };
        d[c] ? d[c]() : Mammamia.renderError(b)
    },
        ...
        b.cartContainer.hide(), b.container.fadeOut(b.fadeSpeed, function() {
            ...
            a.forEach(function(a, e) {
                var f = $("#products-template").html(),
                    g = $(f);
                g.first().addClass(c), Mammamia.insertData(g, a);
                var h = g.find(".Mammamia_getDetail");
    ...
    renderProductOptions: function(a, b) {
        ...
        return a.forEach(function(b) {
            if (Mammamia.productPageOptions in b) Mammamia.renderProductPageOptions(b);
            else {
                var d = "",
    ...
    renderSingleProduct: function(a, b) {
        b.container.fadeOut(b.fadeSpeed, function() {
            var c = $("#product-detail-template").html(),
                d = $(c);
            Mammamia.products.forEach(function(c) {
                if (c.id == a) {
                    if (Mammamia.insertData(d, c), b.container.html(d), c.options.length) {
                        var e = Mammamia.renderProductOptions(c.options, b);
                        $(".Mammamia_options").append(e)
                    }
    ...
    handleFailure: function(a, b) { setTimeout(function() { Mammamia.renderError(a, b) }, 1e3) },
    initJSON: function(a) {
        var b = 'There was an error loading the JSON file. Please make sure you have "' + a.JSONFile + '" file in your main directory.';
        $.get(a.JSONFile).success(function() { $.getJSON(a.JSONFile, function(a) { Mammamia.setProducts(a.products) }).fail(function() { Mammamia.handleFailure(a, b) }) }).fail(function() { Mammamia.handleFailure(a, b) })
    },
    ...
    validatePrices: function(a) {
        ...
        if ("JSON" === a.mode) $.get(a.JSONFile).success(function() {
            $.getJSON(a.JSONFile, function(d) {
                var e = d.products;
                Mammamia.verifyCheckoutData(b, e, !0) ? Mammamia.checkout(a, b) : Mammamia.renderError(a, c)
            }).fail(function() { Mammamia.handleFailure(a, c) })
        }).fail(function() { Mammamia.handleFailure(a, c) });
        else {
            var d = a.mode.toLowerCase();
            Mammamia.plugins[d] && Mammamia.plugins[d].validate(b)
        }
    },
    setProducts: function(a, b) { a.length > 0 && a.forEach(function(a, b) { a.id = b + 1, Mammamia.products.push(a) }), $(window).trigger("hashchange") },
    ...
    generateStore: function() {
        var a = this.settings;
        switch (this.checkMode(a), $(window).on("hashchange", function() { Mammamia.render(window.location.hash, a) }), a.mode) {
            case "JSON":
                this.initJSON(a);
                break;
            case "Google":
                if (Mammamia.plugins.google) Mammamia.plugins.google.init(function(b) { Mammamia.setProducts(b, a) });
                else {
                    var b = "There was an error loading the Google plugin. Make sure it is installed properly.";
                    Mammamia.renderError(a, b)
                }
                break;
            default:
                this.initJSON(a)
        }
        this.generateCart(a), this.setLayout(a), $(".Mammamia_checkout").on("click", function(b) { b.preventDefault(), Mammamia.validatePrices(a) }), $(".Mammamia_viewCart").on("click", function(a) { a.preventDefault(), window.location = "#cart" }), $(".view_close").on("click", function(a) { a.preventDefault(), window.location.hash = "" })
    },
    init: function(a) { return $.isPlainObject(a) ? this.extend(this.settings, a, function() { Mammamia.generateStore() }) : void 0 }
};